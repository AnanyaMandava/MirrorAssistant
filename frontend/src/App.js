...
class App extends Component {
    ...
    this.state = {
      load: {
        ...
        video: {
          ...
          displacement: 0,
          emotions: { angry: 0, fear: 0, happy: 0, neutral: 0 },
          frames: 0,
        },
      },
      currentTalkSpeed: [],
  ...
  queryData = () => {
    ...
    xhr.onreadystatechange = function () {
      if (xhr.readyState == XMLHttpRequest.DONE) {
        ...
        if (
          JSON.parse(xhr.responseText).data.audio.wpm_by_line[
            JSON.parse(xhr.responseText).data.audio.wpm_by_line.length - 1
          ] !== undefined
        ) {
          copySpeed.push({
            x: this.state.currentTalkSpeed.length + 1,
            y: JSON.parse(xhr.responseText).data.audio.wpm_by_line[
              JSON.parse(xhr.responseText).data.audio.wpm_by_line.length - 1
            ],
          });
          newTotal++;
          newSum += JSON.parse(xhr.responseText).data.audio.wpm_by_line[
            JSON.parse(xhr.responseText).data.audio.wpm_by_line.length - 1
          ];
        }
        this.setState({
    ...
    xhr.open("GET", "http://127.0.0.1:5000/current", true);
    xhr.send(null);
  };
  calculateScore = () => {
    let value = 100;
    ...
    let score =
      this.state.load.video.displacement / (10 * this.state.load.video.frames);
    if (score > 8) {
      value -= 60 * ((score - 8) / 8);
    } else if (score < 5) {
      value -= 60 * ((5 - score) / 5);
  ...
  componentDidMount() {
    ...
    xhr.open("GET", "http://127.0.0.1:5000/start", true);
    xhr.send(null);
  }
  ...
  render() {
    ...
    return (
      <div className="App">
        <Welcome
          callback={() => {
            scrollToComponent(this.RT.current, {
              offset: 0,
              align: "top",
              duration: 900,
              ease: "inCirc",
        ...
        <RealTime
          ...
          data={{
            score:
              this.state.load.video.displacement /
              (10 * this.state.load.video.frames),
            expression: this.state.load.video.currentEmotion,
            filler: this.sumCount(this.state.load.audio.counts),
            speed: this.state.currentTalkSpeed,
          }}
          callback={() => {
            scrollToComponent(this.SMRY.current, {
              offset: 0,
              align: "top",
              duration: 900,
              ease: "inCirc",
            });
            clearInterval(this.state.querying);
          }}
        />
        <Summary
          ref={this.SMRY}
          summaryData={{
            expression: {
              happy:
                (this.state.load.video.emotions.happy /
                  this.sumExpressions(this.state.load.video.emotions)) *
                100,
              surprise:
                (this.state.load.video.emotions.surprise /
                  this.sumExpressions(this.state.load.video.emotions)) *
                100,
              neutral:
                (this.state.load.video.emotions.neutral /
                  this.sumExpressions(this.state.load.video.emotions)) *
                100,
              sad:
                (this.state.load.video.emotions.sad /
                  this.sumExpressions(this.state.load.video.emotions)) *
                100,
              angry:
                (this.state.load.video.emotions.angry /
                  this.sumExpressions(this.state.load.video.emotions)) *
                100,
            },
            filler: {
